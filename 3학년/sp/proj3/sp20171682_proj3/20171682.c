...
int main() {
	...
	while (1) {
		...
		case 13:{
			// progaddr [address]의 경우 
			sscanf(input,"%s %s %s",instruction,progadr,check);
			if(check[0] != 0){ printf("Input Error!\n"); break;}
			progaddr = toHex(progadr);
			if(progaddr <0 || progaddr >= 1048576){
				printf("Input Error!\n"); break;
			}
			strcpy(history_input,instruction);
			strcat(strcat(history_input," "),progadr);
			history(history_input);
			break;
		}
		case 14:{
			//loader [object filename1][object filename2][...]의 경우 
			num=0;
			obj[0][0] = obj[1][0]=obj[2][0]=check[0]=0;
			sscanf(input,"%s %s %s %s %s",instruction,obj[0],obj[1],obj[2],check);
			if(check[0] != 0){ printf("Input Error!\n"); break;}
			if(obj[2][0]!=0) num=3;
			else if(obj[1][0]!=0)num=2;
			else if(obj[0][0]!=0)num=1;
			if(progaddr !=csaddr) free_L();
			loader(obj,num);
			A=X=B=S=T=0;				//레지스터 값 초기화
			L = csaddr;//-progaddr;
			PC = progaddr;				//L과 PC값 설정
			bp_start = progaddr;	
			strcpy(history_input,instruction);
			for(int i=0;i<num;i++){
				strcat(strcat(history_input," "),obj[i]);
			}
			history(history_input);
		...
		case 15:{
			...
			if(check[0]==0){			//bp의 경우
				...
				sscanf(input,"%s %s %s",instruction,mnemonic,check);
				if(check[0] !=0){ printf("Input Error!\n");break;}
				if(strcmp(mnemonic,"clear")==0){				//bp clear의 경우
					clear_bp();
				}
				else{		// bp [address]의 경우
					if(isHex(toHex(mnemonic),0)==0){ printf("Input Error!\n"); break;}
					insert_bp(mnemonic);	
				}
				strcpy(history_input,instruction);
				strcat(strcat(history_input," "),mnemonic);	
...
void clearReg(int reg){			//레지스터 번호를 매개변수로 받아 해당 레지스터를 0으로clear한다.
	switch(reg){
		case 0: 
			A=0;
			break;
...
int getReg(int reg){		// 레지스터의 번호를 매개변수로 받아 해당 레지스터의 값을 출력한다.
	switch(reg){
		case 0: return A;
			break;
		case 1: return X;
...
void run(){					//메모리에 load된 프로그램을 실행한다.
	...
	while(PC>=start && PC<end){
		op = memory[PC] - memory[PC]%4;				// opcode와 ni의 값을 구한다.
		ni = memory[PC]%4;
		xbpe[3] = (memory[PC+1]/16)%2;				//순서대로 e,p,b,x 비트의 정보를 저장
		xbpe[2] = (memory[PC+1]/16/2)%2;			
		xbpe[1] = (memory[PC+1]/16/4)%2;
		xbpe[0] = (memory[PC+1]/16/8)%2;

		reg1 = memory[PC+1]/16;					//format 2에 사용될 수 있는 reg 번호
		reg2 = memory[PC+1]%16;
		m =0;
		
		if(ni==0){		//일반 SIC 명령어 형태
			x = memory[PC+1]/128;		//index bit 확인
			m = (memory[PC+1]%128)*16*16 + memory[PC+2];
			if(x==1) m += X;				//SIC machine에서 indexing모드 
			inc =0;							// format 3이므로 0
		}
		else if(xbpe[3]==1){			//format 4인 경우
			m = (memory[PC+1]%16)*16*16*16*16 + memory[PC+2]*16*16 + memory[PC+3];	
			if(memory[PC+1]%16>=8){			//음수인 경우
				m -= 1048576;
			}
			if(xbpe[0]==1) m += X;			// indexing 모드인 경우
			if(ni==2){		//indirect addressing
				cur = m;
				m = memory[cur]*(16*16*16*16)+memory[cur+1]*(16*16)+memory[cur+2];
			}
			inc=1;					//format 4이므로 1
		}
		else{						// format 3인 경우
			m = (memory[PC+1]%16)*16*16 + memory[PC+2];
			if(memory[PC+1]%16>=8) m -= 4096;		//음수인 경우
			if(xbpe[0]==1) m += X;		//indexing mode의 경우
			if(xbpe[1] ==1) m += B;					//base relative 연산의 경우
			else if(xbpe[2]==1) m+= PC+3;			//PC relative연산의 경우
			if(ni==2){			//indirect addressing
				cur = m;
				m = memory[cur]*(16*16*16*16)+memory[cur+1]*(16*16)+memory[cur+2];
			}
		...
		switch(op){
			...
			case 160:			//COMPR
				//그냥 넘어가기 
				if(getReg(reg1)==getReg(reg2))CC=0;
				else CC=-1;
				PC += 2;
				break;
			case 40:			//COMP
			...
			case 80:			//LDCH
				A -= A%256;			//A의 rightmost byte에 값을 넣어주기 위해 비워준다.
				if(ni==3){
					cur = m;
					m = memory[cur];
				}
			...
			case 12:		//STA
				memory[m]=A/65536;
				memory[m+1] = (A%65536)/256;
				memory[m+2] = (A%65536)%256;
				PC += 3+inc;
				
				break;
			case 20:			//STL
				memory[m]=L/65536;
				memory[m+1] = (L%65536)/256;
				memory[m+2] = (L%65536)%256;
				PC += 3+inc;
				break;
			case 16:		//STX
				memory[m]=X/65536;
				memory[m+1] = (X%65536)/256;
				memory[m+2] = (X%65536)%256;
				PC += 3+inc;
				break;
			...
			case 184:			//TIXR
				X++;
				if(X== getReg(reg1))CC=0;
				else if(X > getReg(reg1)) CC=1;
				else CC=-1;
				PC += 2;
				break;
...
void print_reg(){				//현재 레지스터의 상태를 출력한다.
...
void print_bp(){			//sicsim에 존재하는 breakpoint를 전부 화면에 출력한다.
...
void clear_bp(){			//sicsim에 존재하는 breakpoint를 전부 삭제한다. 
	BP* cur,*prev=NULL;
...
void insert_bp(char* bp){					//sicsim에 breakpoint를 지정한다.

	int flg=1;
	BP* new = (BP*)malloc(sizeof(BP));
	BP* cur;
	new -> bp = toHex(bp);
	if(bps == NULL){						//아직 지정된 breakpoint가 없는 경우
		bps = new;
		bp_ptr = bps;
		new->link = new;
		BP_num+=1;
		...
		cur = bps;
		while(cur->link!= bps){
			if(cur->bp == new->bp) { flg=0; break;}			//중복된 breakpoint가 있는 경우 종료
			cur = cur->link;
		}
		if(flg==1) {
			if(cur->bp != new->bp){
...
void free_L(){				//estab에사용되었던  동적할당된 메모리를 해제한다.
...
int find_estab(char* target){			// estab에서symbol의 주소를 찾아준다.	
	...
	for(i=0;i<3;i++){
		cur = estab[i];
		while(cur != NULL){
			if(strncmp(target,cur->name,len)==0){
				return cur->addr;
			}
			cur = cur->link;
		}
...
int char_to_hex(char x){		//char형 문자를 16진수 형태에 맞게 리턴
	
	if(x>='A' && x<='F') return x-'A'+10;
	else return x-'0';
	
}
void load(char* filename){			//메모리에 프로그램을 load한다.

	FILE* fp = fopen(filename,"r");			//load할 obj파일에 접근
	char refnum[13][7],op;					
	char line[100],name[7],pglen[7];
	int addrI,len,offset,ref;
	int i,j,tmp2,tmp3;
	
	fgets(line,100,fp);
	sscanf(line,"H%6s%*6s%6s",name,pglen);			//H라인 읽는다.
	strncpy(refnum[0],name,strlen(name));		//reference number 0은 항상 control section
	fgets(line,100,fp);
	if(line[0] =='D'){							//D라인 읽는다.
		fgets(line,100,fp);
	}
	...
	if(line[0]=='R'){							//R라인 읽는다.
		for(i=1;i<len-1;i+=8){
			sscanf(line+i,"%2X%6s",&ref,name);
			strncpy(refnum[ref-1],name,strlen(name));			//reference number 할당	
			refnum[ref-1][strlen(name)]=0;		
		}
		fgets(line,100,fp);
	}
	while(1){									//T라인 읽는데, obj 코드를 메모리에 1차적으로 load
		if(line[0]=='T'){
			sscanf(line+1,"%6X%2X",&addrI,&len);		
			addrI += csaddr;				
			for(j=0,i=0;i<2*len;i+=2,j++){
				memory[addrI+j]= char_to_hex(line[i+9])*16 + char_to_hex(line[i+10]);
			}
		}
		fgets(line,100,fp);
		if(line[0]=='M' || line[0]=='E') break;			//T 라인이 끝나면 break
	}
	if(line[0]=='M'){								//M 라인 읽는다.
		while(1){
			sscanf(line+1,"%6X%2X%c%2X",&addrI,&offset,&op,&ref);
			addrI += csaddr; 
			tmp2 = find_estab(refnum[ref-1]);				//reference number에 맞는 symbol의 주소 저장
			if(tmp2==-1){ printf("Error!\n"); break;}	
			if(offset %2==1){					// modify할 half byte의 개수가 5인 경우 
				tmp3 = (memory[addrI]%16)*16*16*16*16 + memory[addrI+1]*16*16 + memory[addrI+2];
				if(memory[addrI]%16>=8){			//음수인 경우 처리
					tmp3 -= 1048576;
				}
			}
			else{								//modify할 half byte의 개수가 6인 경우
				tmp3 = memory[addrI]*16*16*16*16 + memory[addrI+1]*16*16 + memory[addrI+2];
				if(memory[addrI]>=128){			//음수인 경우 처리
					tmp3 -= 16777216;
				}
			}
			if(op=='+'){		
			...
			memory[addrI+2] = tmp3 %256;			//load된 코드를 modify해준다.
			tmp3 /= 256;
			memory[addrI+1] = tmp3%256;
			tmp3 /= 256;
			if(offset%2==1) memory[addrI] += tmp3%256;		
			else memory[addrI] = tmp3%256;
			fgets(line,100,fp);
			if(line[0]=='E') break;
		}
	}
	fclose(fp);
	sscanf(pglen,"%X",&len);				
	csaddr += len;						//csaddr은 현재 control section의 길이만큼 더해진다.
	return;
}
void L_pass2(char(*obj)[15],int num){			//loading, relocating, linking 작업 수행 
	
	int i;
	csaddr = progaddr;					
	for(i=0;i<num;i++){
		load(obj[i]);					//control section개수 만큼 loop 돌면서 load 
	}
	return;
}
void loader(char(*obj)[15],int num){			//메모리에 프로그램을 load하기 위해 pass1과 pass2를 호출한다.

	csaddr=progaddr;
	L_pass1(obj,num);	
	L_pass2(obj,num);
}
void L_pass1(char(*obj)[15],int num){		//estab을 생성하는 함수		
	
	int i;
	for(i=0;i<num;i++){
		if(make_estab(obj[i],i)==0){
			free_L();
			printf("L_pass1 error!\n");
			return;
		}
	}
	print_loadmap(num);
...
void print_loadmap(int num){				//생성된 estab을 화면에 출력해주는 함수
...
int insert_estab(char* name,int flag,int addr,int length,int idx){		//estab에 새로운 symbol을 삽입
	...
	if(estab[idx]==NULL){
		...
		while(cur->link != NULL) {
			if(strcmp(name,cur->name)==0) {
				free(new);
				return 0;				//중복된symbol이 있을 경우 에러 처리
...
int make_estab(char* filename,int idx){

	FILE* fp = fopen(filename,"r");
	char line[100];
	char name[8],addrI[8];
	int i,addr,length,len,pgend;
	if(fp==NULL){
		printf("Error! file doesn't exist!!\n");
		return 0;
	}
	fgets(line,100,fp);
	sscanf(line,"H%6s%6X%6X",name,&addr,&length);				//H 라인 읽는다.
	if(insert_estab(name,0,addr+csaddr,length,idx)==0){
		printf("Error! duplicate control section exists\n");		//중복된 symbol 에러 처리
		return 0;	
	}
	...
	len = strlen(line);
	if(line[0]=='D'){											//D라인 읽는다.
		for(i=1;i<len-1;i+=12){
			strncpy(name,line+i,6);
			strncpy(addrI,line+i+6,6);
			sscanf(addrI,"%X",&addr);						//주소는 문자형태로 저장되어 있기때문에 16진수 형태로 다시 받는다.
			if(insert_estab(name,1,addr+csaddr,0,idx)==0){			//현재 읽은 symbol estab에 삽입
				printf("Error! duplicate symbol exists\n");			//중복된 symbol 에러 처리
				return 0;
			}
		}
	}
	while(1){
		if(line[0]=='E'){						//E라인 읽는다.
			sscanf(line+1,"%6X",&pgend);
			csaddr += (estab[idx]->length);		//현재 control section의 길이만큼 csaddr에 더해준다.
			break;
		}
...
int toHex(char* str){				//문자열을 16진수의 수로 리턴
	...
	for(i=len-1;i>=0;i--,mul*=16){
		...
		else if(str[i]>='a' && str[i]<='f') sum += (str[i]-'a'+10)*mul;
...
int check_input(char* inputstr) {							//입력된 문자열이 제공하는 shell 명령어에 부합하는지 확인
	...
	else if(!strcmp(inputstr,"progaddr")){
		return 13;
	}
	else if(!strcmp(inputstr,"loader")){
		return 14;
	}
	else if(!strcmp(inputstr,"bp")){
		return 15;
	}
	else if(!strcmp(inputstr,"run")){
		return 16;
	}
	else return -1;
}
