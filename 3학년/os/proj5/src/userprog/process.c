...
//	page_init(&thread_current()->supple_pt);	
...
void close_files(){
	...
	for(int i=0;i<128;i++){
		if(cur->fd[i] != NULL){
			file_close(cur->fd[i]->file);			//프로세스가 exit하기 전에 사용했던 file들이 아직 남아있으면 close한다.
			free(cur->fd[i]);
			cur->fd[i] = NULL;
		}
	}
...
/*
	...
	if(pg == NULL) 
		...
		if(!insert_page(&cur->supple_pt,&pg->elem))
			return false;
*/
	  ...
	  //ofs += page_read_bytes;//page_read_bytes;
...
setup_stack (void **esp) 
{
  ...
  if (kpage != NULL) 
    {
	  ...
	  if((size_t)(PHYS_BASE - pg_round_down((uint8_t*)PHYS_BASE-PGSIZE))> 1<<23)
		  return false;
  ...
  /*
	...
	pg->addr = pg_round_down((uint8_t*)(PHYS_BASE) - PGSIZE);
	pg->is_loaded = true;
	pg->writable = true;
	...
	page_insert(&thread_current()->supple_pt,kpage->pg);
*/
	return success;
//  return true;
}
