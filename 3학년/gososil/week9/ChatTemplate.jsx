...
function ChatTemplate() {
	...
	const getChatLog = () => {
		...
		const url = "http://localhost:8001/get"

		axios.get(url).then((res) => {
			if(chat_len < res.data.length){
				setChats(res.data)
				setChatLen(res.data.length)
			}
		})
		// 9주차 실습 구현
	}
	...
	const onSearchKeySubmit = (keyword) => {
		...
		for(var i =0;i<chat_len;i++){
			// alert(chats[i].message)
			chats[i].searched = false;
			if(chats[i].message.includes(keyword)){
				chats[i].searched = true;
				// alert("onserat")
			}
		}
	}
