...
function TodoTemplate() {
  ...
  const onToggleToDo = (id) => {
    // 7주차 실습 구현
    // 체크 안 된 항목을 todo 배열에서 찾음
    const checkedTodo = todos.find((todo)=>todo.checked==false);
    // 체크 표시
    checkedTodo.checked = true;
    // 체크한 todo -> donetodos에 저장
    setDoneTodos([...doneTodos,checkedTodo]);
    // filter 함수 사용해서 id에 해당하는 항목만 todo 배열에서 제외
    setTodos(todos.filter((todo)=>todo.id !== id));
  };
  ...
  const onToggleDoneToDo = (id) => {
    // 7주차 실습 구현
    // id에 해당하는 항목을 donetodo 배열에서 찾음
    const checkedDoneTodo = doneTodos.find((todo)=>todo.id == id);
    // 체크 표시 해제
    checkedDoneTodo.checked = false;
    // 제거한 doneTodo -> todos에 저장
    setTodos([...todos,checkedDoneTodo]);
    // filter 함수 사용해서 id에 해당하는 항목만 donetodo 배열에서 제외
    setDoneTodos(doneTodos.filter((todo)=>todo.id !== id));
